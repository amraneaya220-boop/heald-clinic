...
<?php $__env->startSection('title', 'Medical Ads'); ?>
...
<script>
    ...
    async function fetchAds() {
        try {
            const response = await fetch('<?php echo e(route("clinic.announcements.data")); ?>');
    ...
    async function showDetails(id) {
        try {
            const response = await fetch(`<?php echo e(url("clinic/announcements")); ?>/${id}`);
            const data = await response.json();
            if (data.success) {
                const ad = data.data;
    ...
    async function publishNewAd() {
        ...
        try {
            const response = await fetch('<?php echo e(route("clinic.announcements.store")); ?>', {
                method: 'POST',
                headers: {
...
<?php echo $__env->make('clinic.layouts.clinic', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\heald-clinic\resources\views/clinic/announcements/index.blade.php ENDPATH**/ ?>