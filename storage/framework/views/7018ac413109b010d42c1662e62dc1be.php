...
<script>
    ...
    async function fetchDoctors() {
        ...
        const res = await fetch(`<?php echo e(route("clinic.doctors.data")); ?>?search=${search}&status=${status}`);
...
console.log('Working days value:', document.getElementById('docDays').value);
    ...
    async function deleteDoctor(id) { if(confirm('Delete?')) { await fetch(`<?php echo e(url('clinic/doctors')); ?>/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' } }); fetchDoctors(); } }
    async function editDoctor(id) { const newName = prompt('Enter new name:'); if(newName) { await fetch(`<?php echo e(url('clinic/doctors')); ?>/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' }, body: JSON.stringify({ name: newName }) }); fetchDoctors(); } }
    async function toggleDoctorStatus(id) { await fetch(`<?php echo e(url('clinic/doctors')); ?>/${id}/toggle-status`, { method: 'POST', headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' } }); fetchDoctors(); }
    async function openScheduleModal(doctorId) { currentDoctorId = doctorId; const res = await fetch(`<?php echo e(url('clinic/doctors')); ?>/${doctorId}/schedule`); const data = await res.json(); if(data.success) { currentWorkSchedule = data.data.workSchedule; currentTimeSlots = data.data.timeSlots; renderScheduleModal(); document.getElementById('scheduleModal').style.display = 'flex'; } }
...
<?php echo $__env->make('clinic.layouts.clinic', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\heald-clinic\resources\views/clinic/doctors/index.blade.php ENDPATH**/ ?>