...
<div class="plans-container">
    ...
    <div class="trial-banner">
        <div class="trial-text">
            ...
            <div>
                <h3>{{ $remainingTrials }} Free Trial{{ $remainingTrials > 1 ? 's' : '' }} Available!</h3>
                <p>Test all features before committing to a subscription</p>
            </div>
        </div>
        <form action="{{ route('clinic.subscription.use-trial') }}" method="POST" style="display: inline;">
            @csrf
    ...
    <div class="plans-grid">
        ...
        <div class="plan-card {{ $key == 'yearly' ? 'popular' : '' }}">
            @if($key == 'yearly')
                <div class="popular-badge">⭐ Most Popular</div>
            @endif
            <h3>{{ $plan['name'] }}</h3>
            <div class="price">{{ number_format($plan['price'], 2) }} <span style="font-size: 18px;">DZD</span></div>
            <div class="price-period">per {{ $plan['duration'] }}</div>
            ...
            <button class="btn-subscribe" onclick="openPaymentModal('{{ $key }}', '{{ $plan['name'] }}', {{ $plan['price'] }})">
...
<div id="paymentModal" class="payment-modal" style="display: none;">
    <div class="payment-modal-content">
        ...
        <form id="paymentForm" method="POST" action="{{ route('clinic.subscription.subscribe') }}">
            @csrf
