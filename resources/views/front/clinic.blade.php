...
<div class="card">
    <div class="image">
        <img src="{{ $clinic->image ?? 'https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?w=600&h=350&fit=crop' }}" alt="{{ $clinic->name }}">
    </div>
    <div class="content">
        <h1>{{ $clinic->name }}</h1>
        <div class="location">📍 {{ $clinic->location }}</div>
        <div class="stars">{!! str_repeat('⭐', round($clinic->rating ?? 4)) !!} ({{ $clinic->rating ?? 4 }}.0)</div>
        <div class="divider"></div>
        <div class="buttons">
            <button class="back" onclick="history.back()">← Back</button>
            <button class="details" onclick="window.location.href='{{ route('clinic.details', $clinic->id) }}'">📋 Show Details</button>
        </div>
    </div>
</div>
@endsection