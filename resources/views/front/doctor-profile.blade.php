...
<head>
    ...
    <title>{{ $doctor->name }} - MediEase</title>
...
<div class="navbar">
    ...
    <div class="nav-center"><a href="{{ url('/') }}" id="homeLink">Home</a></div>
    <div class="lang">
...
<div class="card">
    <div class="header">
        ...
        <p id="docSpecialty">{{ $doctor->specialty }}</p>
    </div>
    <div class="content">
        <div class="section">
            <h3 id="personalInfoTitle">📋 Personal Information</h3>
            <div class="row"><div class="label" id="fullNameLabel">Full Name</div><div class="value" id="fullName">{{ $doctor->name }}</div></div>
            <div class="row"><div class="label" id="specialtyLabel">Specialty</div><div class="value" id="specialty">{{ $doctor->specialty }}</div></div>
            <div class="row"><div class="label" id="clinicLabel">Clinic</div><div class="value" id="clinic">{{ $doctor->clinic->name ?? '' }}</div></div>
            <div class="row"><div class="label" id="experienceLabel">Experience</div><div class="value" id="experience">{{ $doctor->experience ?? '10+ years' }}</div></div>
            <div class="row"><div class="label" id="hoursLabel">Working Hours</div><div class="value" id="hours">{{ $doctor->working_hours ?? 'Saturday - Thursday: 9:00 AM - 5:00 PM' }}</div></div>
            <div class="row"><div class="label" id="phoneLabel">Phone</div><div class="value" id="phone">{{ $doctor->phone }}</div></div>
        </div>
        <div class="section">
            <h3 id="feeTitle">💰 Consultation Fee</h3>
            <div class="row"><div class="label" id="visitPriceLabel">Visit Price</div><div class="value"><span class="price" id="fee">{{ $doctor->consultation_fee ?? '2500 DZD' }}</span></div></div>
        </div>
        <div class="section">
            ...
            <div class="row"><div class="label" id="ratingLabelMain">Rating</div>
                <div class="value">
                    ...
                    <span id="ratingText">({{ number_format($rating,1) }}/5 - {{ $doctor->reviewCount ?? 0 }} reviews)</span>
                </div>
            </div>
        </div>
        <div class="section">
            <h3 id="aboutTitle">📖 About</h3>
            <div class="value" id="about" style="line-height:1.6;">{{ $doctor->about ?? 'Experienced medical professional.' }}</div>
        </div>
...
<script>
    ...
    function goToLogin(){
        window.location.href = "{{ route('login', ['type'=>'patient']) }}";
    }
    function goToRegister(){
        window.location.href = "{{ route('register', ['type'=>'patient']) }}";
    }
    function bookAppointment(){
        window.location.href = "{{ route('appointment.create', $doctor->id) }}";
    }
    ...
    function displayReviewsExp(){
        fetch('{{ url("/api/reviews/doctor") }}/'+doctorId)
