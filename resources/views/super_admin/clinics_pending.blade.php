                ...
                <div class="card-body">
                    ...
                    <div class="stats-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin-bottom: 30px;">
                        <div class="stat-card" style="background: #fef3c7; border-radius: 20px; padding: 20px; text-align: center;">
                            ...
                            <h3 style="font-size: 32px; margin: 10px 0;">{{ $pendingCount ?? 0 }}</h3>
                        ...
                        <div class="stat-card" style="background: #dcfce7; border-radius: 20px; padding: 20px; text-align: center;">
                            ...
                            <h3 style="font-size: 32px; margin: 10px 0;">{{ $approvedCount ?? 0 }}</h3>
                        ...
                        <div class="stat-card" style="background: #dbeafe; border-radius: 20px; padding: 20px; text-align: center;">
                            ...
                            <h3 style="font-size: 32px; margin: 10px 0;">{{ $totalCount ?? 0 }}</h3>
                            <p style="color: #64748b;">Total Clinics</p>
                        </div>
                    </div>
                    
                    @if(count($pendingClinics) > 0)
                    <div style="overflow-x: auto;">
                        <table class="table" style="width: 100%; border-collapse: collapse;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td style="padding: 12px; border-bottom: 1px solid #e2e8f0;">{{ $clinic->id }}</td>
                                    <td style="padding: 12px; border-bottom: 1px solid #e2e8f0;">
                                        <strong>{{ $clinic->name }}</strong>
                                    </td>
                                    <td style="padding: 12px; border-bottom: 1px solid #e2e8f0;">{{ $clinic->user->email ?? 'N/A' }}</td>
                                    <td style="padding: 12px; border-bottom: 1px solid #e2e8f0;">{{ $clinic->phone ?? 'N/A' }}</td>
                                    <td style="padding: 12px; border-bottom: 1px solid #e2e8f0;">{{ $clinic->location ?? 'N/A' }}</td>
                                    <td style="padding: 12px; border-bottom: 1px solid #e2e8f0;">{{ $clinic->created_at->format('Y-m-d') }}</td>
                                    <td style="padding: 12px; border-bottom: 1px solid #e2e8f0;">
                                        <button class="btn-approve" onclick="approveClinic({{ $clinic->id }})" style="background: #10b981; color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; margin-right: 8px;">
                                            <i class="fas fa-check"></i> Approve
                                        </button>
                                        <button class="btn-reject" onclick="rejectClinic({{ $clinic->id }})" style="background: #ef4444; color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer;">
